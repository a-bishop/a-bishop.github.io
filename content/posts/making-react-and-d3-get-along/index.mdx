...
Using the same methodology as before, I created a `color` function which applies a value from a range of hex codes for each bar within the graph, based on the value of its population data.

```js
const color = d3.scaleLinear().domain([0, highestVal]).range(['#c19898', '#4a4a48']);
```

Here are the parent SVG `Chart` component and child `Bar` components, which will now be rendered with the desired parameters (note how the color and scale functions are being used):
  ...
  {props.populations.map((country, index) => {
    return (
      <Bar
        ...
        y={MAX_HEIGHT - scale(country.population)}
        width={BAR_WIDTH}
        height={scale(country.population)}
      />
    );
  })}
</Chart>
