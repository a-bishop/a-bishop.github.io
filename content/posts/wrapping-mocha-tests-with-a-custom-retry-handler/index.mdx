...
While writing integration tests in Mocha for the payment platform at Change.org, I helped create a simple higher-order function that would allow us to avoid the use of these heavy-handed timeouts. We wanted to be able to retry the test if it failed, using increasing timeouts calculated on each run.
